...
const (
	...
	compressSuffix   = ".gz"
	zoneSeconds      = 28800 // the time zone of Asia/Shanghai
)
...
// original name. Thus, the filename you give RotateFile is always the "current" log
// file.
//
// Backups use the log file name given to RotateFile, in the form
// `name-timestamp.ext` where name is the filename without the extension,
// timestamp is the time at which the log was rotated formatted with the
// time.Time format of `2006-01-02T15-04-05.000` and the extension is the
// original extension.  For example, if your RotateFile.Filename is
...
type RotateFile struct {
	...
	MaxSize int `json:"maxsize" yaml:"maxsize" form:"size"`

	// Cycle time cycle, one of daily/weekly/monthly
	Cycle string `json:"cycle" yaml:"cycle" form:"cycle"`

	// Minutely minute cycle, from 1 to 720 (half day)
	Minutely int `json:"minutely" yaml:"minutely" form:"min"`
	...
	// based on age.
	MaxAge int `json:"maxage" yaml:"maxage" form:"age"`

	// MaxBackups is the maximum number of old log files to retain.  The default
	// is to retain all old log files (though MaxAge may still cause them to get
	// deleted.)
	MaxBackups int `json:"maxbackups" yaml:"maxbackups" form:"bak"`

	// LocalTime determines if the time used for formatting the timestamps in
	// backup files is the computer's local time.  The default is to use UTC
	// time.
	LocalTime bool `json:"localtime" yaml:"localtime" form:"local"`
...
// Sync implements zap.Sink.SyncWriter
func (l *RotateFile) Sync() error {
	return l.file.Sync()
}
...
func (l *RotateFile) Write(p []byte) (n int, err error) {
	...
	writeLen, maxSize := int64(len(p)), l.max()
	if maxSize > 0 && writeLen > maxSize {
		return 0, fmt.Errorf(
...
func (l *RotateFile) openNew() error {
	...
	if err == nil {
		...
		mode, last = info.Mode(), info.ModTime()
		// move the existing file
		if !l.LocalTime {
			last = last.UTC()
		}
		newName := backupName(name, l.getTimeFormat(), last)
...
func (l *RotateFile) timeFromName(filename, prefix, ext string) (time.Time, error) {
	...
	return time.Parse(l.getTimeFormat(), ts)
}
...
// isDiffTime the time.Time is not in the same cycle
func isDiffTime(secs int64, modTime, nowTime time.Time) bool {
	if secs <= 0 || secs > 720 {
		return false
	}
	modStamp := modTime.Unix() + zoneSeconds
	nowStamp := nowTime.Unix() + zoneSeconds
	diff := nowStamp - modStamp
	if diff < 0 && 0-diff >= secs || diff >= secs {
		return true
	}
	return modStamp/secs != nowStamp/secs
}
